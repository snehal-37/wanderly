...
export default function DestinationCarousel() {
  ...
  return (
    <section id="destinations" className="py-24 sm:py-32 px-6 sm:px-10 max-w-[1500px] mx-auto">
      <ScrollReveal direction="up" className="flex items-end justify-between mb-12 gap-6">
        <div>
          <p className="text-sm text-ink/50 mb-2">A closer look</p>
          <h2 className="font-display text-4xl sm:text-5xl">Where next?</h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 items-stretch">
        {/* Active slide */}
        <div className="relative h-[70vh] sm:h-[75vh] rounded-2xl overflow-hidden bg-sand cursor-grab active:cursor-grabbing">
          <AnimatePresence mode="popLayout" custom={dir}>
            <motion.div
              ...
              initial={{ clipPath: dir > 0 ? 'inset(0 0 0 100%)' : 'inset(0 100% 0 0)', scale: 1.05 }}
            ...
            >
              <img
                ...
                draggable={false}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-ink/75 via-ink/0 to-ink/10" />

              <div className="absolute top-6 left-6 flex items-center gap-3">
                <span className="text-ivory/70 text-xs font-medium tracking-wide">{current.region}</span>
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 text-ivory">
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <p className="text-ivory/75 text-sm font-light mb-5">{current.description}</p>
              ...
              <button className="absolute top-6 right-6 w-10 h-10 rounded-full bg-ivory/15 backdrop-blur-md flex items-center justify-center text-ivory hover:bg-ivory hover:text-ink transition">
        ...
        <div className="flex lg:flex-col gap-4">
          ...
          <div className="flex lg:hidden items-center gap-3">
            <button onClick={() => go(-1)} className="w-11 h-11 rounded-full border border-ink/15 flex items-center justify-center">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <button onClick={() => go(1)} className="w-11 h-11 rounded-full border border-ink/15 flex items-center justify-center">
              <ArrowRight className="w-4 h-4" />
